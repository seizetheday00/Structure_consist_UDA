...
def get_model(model_name, pretrain=True):
    if model_name in models.__dict__:
        ...
        backbone = models.__dict__[model_name](pretrained=pretrain)  #20220812注释 这是原始的
...
# 用download_flag替换掉了TRUE
def get_dataset(dataset_name, root, source, target, download_flag, train_source_transform, val_transform, train_target_transform=None):
    ...
    if dataset_name == "Digits":
        ...
        train_source_dataset = concat_dataset(root=root, tasks=source, download=download_flag, transform=train_source_transform)
        train_target_dataset = concat_dataset(root=root, tasks=target, download=download_flag, transform=train_target_transform)
        val_dataset = concat_dataset(root=root, tasks=target, download=download_flag, transform=val_transform)
        if dataset_name == 'DomainNet':
            test_dataset = concat_dataset(root=root, tasks=target, split='test', download=download_flag, transform=val_transform)
        else:
        ...
        dataset = wilds.get_dataset(dataset_name, root_dir=root, download=download_flag)
...
def validate(val_loader, model, args, device) -> float:
    ...
    with torch.no_grad():
        ...
        # _,_ ,_ ,confmat1 = confmat.compute()
        # confmat1 = confmat1.cpu().numpy()
        
        
        # print(len(args.class_names))
        # _,_ ,_ ,confmat1 = confmat.compute()
        # print(confmat1)
            
    return top1.avg#, confmat1
