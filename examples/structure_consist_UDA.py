...
from dalib.adaptation.spl import StructurePreserveLoss
...
def main(args: argparse.Namespace):
    ...
    if args.log_results:
        ...
        wandb.init(project="DA", entity="yuyingliu", name=args.log_name)
    ...
    # spl loss
    spl_loss= StructurePreserveLoss(num_classes=num_classes, features_dim=classifier_feature_dim)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier, domain_adv, spl_loss, optimizer,
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
          domain_adv: ConditionalDomainAdversarialLoss, spl_loss, optimizer: SGD,
    ...
    spl_loss_value = AverageMeter('Spl Loss', ':3.1f')
    progress = ProgressMeter(
        args.iters_per_epoch,
        [batch_time, data_time, losses, trans_losses, cls_accs, domain_accs, spl_loss_value],
    ...
    for i in range(args.iters_per_epoch):
        ...
        transfer_loss = domain_adv(y_s, f_s, y_t, f_t) + spl_loss(y_s, f_s, y_t, f_t)
        spl_loss_value = spl_loss(y_s, f_s, y_t, f_t)
        ...
        if args.log_results:
            wandb.log({'iteration': epoch*args.iters_per_epoch + i, 'loss': loss, 'cls_loss': cls_loss,
                       ...
                       'domain_acc': domain_acc, 'spl_loss':spl_loss_value})
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Structure Consistent Unsupervised Domain Adaptation')
    # dataset parameters
